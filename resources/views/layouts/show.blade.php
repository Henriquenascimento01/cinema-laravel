        ...
        <div id="cards-container" class="row">
            ...
            @foreach ($sessions as $session)
                <div class="col md-4">
                    ...
                    <img src="/img/movies/{{ $session->movie->image }}">

                    <div class="card-body">
                        <p class="card-date">{{ \Carbon\Carbon::parse($session->date)->format('d/m/Y') }}</p>
                        <h5 class="card-title">{{ $session->movie->name }}</h5>
                        <p class="card-duration">Inicio: {{ \Carbon\Carbon::parse($session->time_initial)->format('H:i') }}
                        </p>
                        <p class="card-duration">Término: {{ \Carbon\Carbon::parse($session->time_finish)->format('H:i') }}
                        </p>
